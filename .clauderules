...
* **Go 语言版 md2wechat CLI 标题字节超限报错**：
  在 Go 中，`len(string)` 方法统计的是字符串的字节数（Byte Length）而非字符数（Rune Length）。在 UTF-8 编码下，每个汉字占用 3 字节。因此，若向 `md2wechat` 命令行参数 `--title` 传递超过 10 个汉字的标题，其字节数即超过 30 字节，将触发 CLI 内部 `len(title) > 32` 的超限校验报错崩溃。
  **修复原则**：在 Python 逻辑调用 `md2wechat` 时，需强制将传入命令行的标题截断至 10 个字符以内。这仅用于 CLI 渲染，并不影响发往微信 API 的真实推文大标题。
* **临时草稿状态丢失与污染防范**：
  为避免刷新页面或切换文章导致微信草稿 ID 丢失而需要重复生成草稿，系统采用推文同目录下对应的 `.draft.json` 关联文件来绑定微信 MediaID 和 URL。在公众号弹窗开启时主动识别加载该配置，切换文章时自动清空历史缓存，杜绝状态相互污染。
