...
def load_accounts(config_path: str = CONFIG_FILE) -> list:
...
def save_accounts(accounts: list, config_path: str = CONFIG_FILE) -> bool:
...
def auto_publish_to_wechat(md_filepath: str, html_filepath: str, account_config: dict, publish_mode: str, author: str = "") -> dict:
    ...
    # 安全防护锁：防止 AI 拒答/错误占位文本误发布到微信
    invalid_keywords = [
        "无法基于", "内容为空", "占位提示", "SYSTEM NOTICE", "系统提示",
        "我无法", "请提供真实", "无法为您生成", "由于你提供", "由于您提供",
        "抱歉，我", "无法处理", "未找到有效", "跳过分析"
    ]
    for kw in invalid_keywords:
        if kw in md_text:
            raise ValueError(f"安全拦截：检测到 Markdown 文件包含 AI 拒答/错误占位关键词 '{kw}'，已安全取消公众号发布！")
