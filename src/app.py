...
def run_manual_deep_analysis_worker(curr_group_id, l_limit, s_key, current_chan_config, a_mode, use_p_ui, cp_text, use_wechat, also_generate_report, wechat_prompt, config):
    ...
    try:
        ...
        if use_wechat:
            ...
            is_src_valid, src_reason = validate_wechat_article_content(final_res)
            if not is_src_valid:
            ...
            wechat_system_prompt = get_wechat_system_prompt(wechat_orientation)
            ...
            if wc_response.choices and wc_response.choices[0].message.content:
                ...
                is_wc_valid, wc_reason = validate_wechat_article_content(raw_wechat)
                if not is_wc_valid:
                    write_manual_log(f"⚠️ 微信推文安全审查未通过 ({wc_reason})，取消保存与发布！")
                    save_manual_task_state(status="error", error=wc_reason)
                    return

                raw_wechat = ensure_image_prompts_exist(raw_wechat)
...
def run_scheduled_deep_analysis(ignore_auto_run: bool = False):
    ...
    try:
        status_res = subprocess.run([NPX_CMD, "zsxq-cli", "auth", "status", "--json"], capture_output=True, text=True, encoding='utf-8', env=get_exec_env())
        user_id = ""
        ...
        if use_wechat:
            ...
            for wc_attempt in range(max_wc_attempts):
                ...
                if wc_response.choices and wc_response.choices[0].message.content:
                    ...
                    is_wc_valid, wc_reason = validate_wechat_article_content(cand_wechat)
                    if is_wc_valid:
...
def fetch_zsxq(group_id, limit=3, scope="all", progress_callback=None):
    ...
    for attempt in range(max_retries):
        ...
        ], capture_output=True, text=True, encoding='utf-8', timeout=45, env=get_exec_env())
                ...
                if fname.lower().endswith(('.pdf', '.docx', '.xlsx', '.csv', '.md')):
                    ...
                    try:
                        ...
                        for dl_attempt in range(3):
                            dl_res = subprocess.run([NPX_CMD, "zsxq-cli", "api", "call", "call_zsxq_api", "--params", json.dumps({"method": "GET", "path": f"/v2/files/{fid}/download_url"})], capture_output=True, text=True, encoding='utf-8', timeout=15, env=get_exec_env())
    ...
    with st.expander("⏰ 定时任务 (Cron)", expanded=False):
        ...
        with col_test:
            if st.button("⚡ 测试运行任务", use_container_width=True):
                ...
                def test_runner():
                    if page_selection == "AI 深度分析":
                        run_scheduled_deep_analysis(ignore_auto_run=True)
                    elif page_selection == "视频脚本制作器":
                        run_scheduled_video_script(ignore_auto_run=True)
                    elif page_selection == "指标文档制作":
                        run_scheduled_indicator_docs(ignore_auto_run=True)
...
if page_selection == "AI 深度分析":
    ...
    with col1:
        ...
        task_state = get_manual_task_state()
        ...
        if is_running:
            ...
            if st.button("🛑 停止当前任务", use_container_width=True, type="primary"):
                request_stop_manual_task()
                write_manual_log("🛑 用户在界面点击了停止当前任务按钮...")
